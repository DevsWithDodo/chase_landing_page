  ...
  return (
    <section className="py-24 px-4 sm:px-6 lg:px-8 bg-surface relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <p className="text-xl text-body-large text-center text-on-surface-variant max-w-3xl mx-auto mb-16 leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 mt-16 max-w-6xl mx-auto">
          {features.map((feature, index) => (
            <div
              key={index}
              className="p-8 bg-surface-container rounded-md transition-all hover:-translate-y-2 hover:shadow-lg border border-outline/10"
            >
              <div className="w-12 h-12 mb-6 bg-primary/10 rounded-sm flex items-center justify-center text-3xl">
                {feature.icon}
              </div>
              <h3 className="text-title-large font-bold mb-4 text-on-surface">{feature.title}</h3>
              <p className="text-body-medium text-on-surface-variant leading-relaxed">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
